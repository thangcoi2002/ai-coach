...
type PickedFile = { name: string; size: string };
  ...
  return (
    <StepLayout badge="Cách 1 · Báo cáo" onBack={onBack}>
      ...
      <Pressable
        accessibilityRole="button"
        onPress={handlePick}
        className="mt-3 items-center rounded-2xl border-[1.5px] border-dashed border-brand-ink/25 p-5">
        ...
        <Text className="mt-2.5 text-[15px] font-bold text-brand-ink">
          {pickedFile ? 'Chọn tệp khác' : 'Tải tệp lên'}
        </Text>
      ...
      {pickedFile && (
        <View className="mt-3 rounded-2xl bg-brand-card p-4">
          <View className="flex-row items-center justify-between gap-2.5">
            <Text className="min-w-0 flex-1 text-[15px] font-bold text-brand-ink" numberOfLines={1}>
              {pickedFile.name}
            </Text>
            {!!pickedFile.size && (
      ...
      <View className="mt-5">
        <PrimaryButton
          label="Dùng kết quả này"
          onPress={() => onNext(report.sourceLabel)}
          disabled={!pickedFile}
        />
      </View>
    </StepLayout>
