        ...
        {STATS.map(stat => (
          <View key={stat.key} className={`flex-1 gap-2.5 p-3.5 ${cardClassName}`}>
            <View
              ...
              style={{ backgroundColor: isDark ? `${stat.iconColor}33` : stat.iconBg }}>
              <stat.Icon size={18} color={stat.iconColor} />
            </View>
            <View>
              <Text className="text-2xl font-semibold tracking-[-0.02em] text-brand-ink dark:text-brandDark-ink">
                {stat.value}
              </Text>
              <Text className="text-xs text-brand-body dark:text-brandDark-body">{stat.label}</Text>
            </View>
          </View>
        ))}
      </View>
